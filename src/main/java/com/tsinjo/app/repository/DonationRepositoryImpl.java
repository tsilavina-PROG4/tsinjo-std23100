...
public class DonationRepositoryImpl implements DonationRepository {
  ...
  private static final RowMapper<Donation> DONATION_ROW_MAPPER =
      new RowMapper<>() {
        @Override
  ...
  public Donation save(Donation donation) {
    if (donation.getId() == null) {
      String sql =
          "INSERT INTO donation (donor_id, amount, date, payment_id) VALUES (?, ?, ?, ?) RETURNING"
              + " id";
      Long id =
      ...
      String sql =
          "UPDATE donation SET donor_id = ?, amount = ?, date = ?, payment_id = ? WHERE id = ?";
      jdbcTemplate.update(
          sql,
          donation.getDonor() != null ? donation.getDonor().getId() : null,
          donation.getAmount(),
