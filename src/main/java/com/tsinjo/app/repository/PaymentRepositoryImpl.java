...
public class PaymentRepositoryImpl implements PaymentRepository {
        ...
        public Payment mapRow(ResultSet rs, int rowNum) throws SQLException {
          return Payment.builder()
              ...
              .lastVerificationInstant(
                  rs.getTimestamp("last_verification_instant") != null
  ...
  public Payment save(Payment payment) {
    if (payment.getId() == null) {
      String sql =
          "INSERT INTO payment (psp_payment_id, payer_email, psp_type, status, creation_instant,"
              + " last_verification_instant, amount) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id";
      Long id =
          jdbcTemplate.queryForObject(
              ...
              payment.getPspType(),
              payment.getStatus(),
              payment.getCreationInstant(),
              payment.getLastVerificationInstant(),
              payment.getAmount());
      payment.setId(id);
    } else {
      String sql =
          "UPDATE payment SET psp_payment_id = ?, payer_email = ?, psp_type = ?, status = ?,"
              + " creation_instant = ?, last_verification_instant = ?, amount = ? WHERE id = ?";
      jdbcTemplate.update(
          sql,
          payment.getPspPaymentId(),
          payment.getPayerEmail(),
